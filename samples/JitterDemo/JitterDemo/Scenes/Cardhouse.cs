...
namespace JitterDemo.Scenes
{
    class CardHouse : Scene
    {
        ...
        static Vector3 cardHouseStartingPosition = new Vector3(0, 0, 0);
            ...
            for (int layer = 0; layer < cardHouseLayers; layer++)
            {
                ...
                AddCardLayer(
                    cardHouseStartingPosition +
                        new Vector3(cardSpacing * layer, (layerHeight + (float)(2 * cardThickness)) * layer, 0),
                    layerCards);
            }
        }

        private void AddCardLayer(Vector3 startPosition, int angledCards)
        {
            ...
            {
                AddCard(
                    startPosition + new Vector3(cardSpacing * i, layerHeight / 2f, 0),
                    (i % 2 == 0) ? angle : oppositeAngle);
            }

            for (float distance = 1.5f; distance < angledCards - 0.5; distance += 4)
            {
                AddCard(startPosition + new Vector3(cardSpacing * distance, layerHeight, 0), 0);
            }

            for (float distance = 3.5f; distance < angledCards - 0.5; distance += 4)
            {
                AddCard(startPosition + new Vector3(cardSpacing * distance, layerHeight + (float)cardThickness, 0), 0);
            }
        }

        private void AddCard(Vector3 position, float rollOrientation)
        {
