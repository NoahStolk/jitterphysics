    ...
    public class World
    {
        ...
        private void UpdateArbiterContacts(Arbiter arbiter)
        {
            ...
            for (int i = arbiter.contactList.Count - 1; i >= 0; i--)
            {
                ...
                {
                    Vector3 diff = Vector3.Subtract(c.p1, c.p2);
        ...
        private void IntegrateForces()
        {
            foreach (RigidBody body in rigidBodies)
            {
                if (!body.isStatic && body.IsActive)
                {
                    ...
                    Vector3 temp = Vector3.Multiply(body.force, body.inverseMass * timestep);
                    body.linearVelocity = Vector3.Add(temp, body.linearVelocity);

                    if (!(body.isParticle))
                    {
                        temp = Vector3.Multiply(body.torque, timestep);
                        temp = Vector3.Transform(temp, body.invInertiaWorld);
                        body.angularVelocity = Vector3.Add(temp, body.angularVelocity);
                    }

                    if (body.affectedByGravity)
                    {
                        temp = Vector3.Multiply(gravity, timestep);
                        body.linearVelocity = Vector3.Add(body.linearVelocity, temp);
                    }
                }
        ...
        private void IntegrateCallback(object obj)
        {
            ...
            Vector3 temp = Vector3.Multiply(body.linearVelocity, timestep);
            body.position = Vector3.Add(temp, body.position);
            ...
            if (!(body.isParticle))
            {
                ...
                if (angle < 0.001f)
                {
                    ...
                    axis = Vector3.Multiply(body.angularVelocity, (0.5f * timestep - (timestep * timestep * timestep) * (0.020833333333f) * angle * angle));
                }
                else
                {
            ...
            if ((body.Damping & RigidBody.DampingType.Linear) != 0)
                body.linearVelocity = Vector3.Multiply(body.linearVelocity, currentLinearDampFactor);

            if ((body.Damping & RigidBody.DampingType.Angular) != 0)
                body.angularVelocity = Vector3.Multiply(body.angularVelocity, currentAngularDampFactor);
